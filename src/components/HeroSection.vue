<template>
  <section 
    ref="sectionRef" 
    class="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-mulyo-cream"
  >
    <!-- Background Wrapper -->
    <div 
      ref="bgRef" 
    ...
    >
      ...
      <div class="absolute inset-0 bg-gradient-to-t from-mulyo-cream via-transparent to-transparent opacity-80"></div>
    </div>
    ...
    <div class="relative z-10 flex flex-col items-center justify-center text-center px-4 w-full">
       <div ref="el => { if(el) textRefs[0] = el }" class="opacity-0 invisible">
         <p class="text-mulyo-gold tracking-[0.4em] text-xs md:text-sm font-sans font-medium mb-6 uppercase">
           The Wedding Of
         </p>
       </div>
       
       <div ref="el => { if(el) textRefs[1] = el }" class="opacity-0 invisible relative">
         <h1 class="text-mulyo-charcoal text-5xl md:text-8xl lg:text-9xl font-playfair mb-4">
           <span class="absolute -top-10 -left-10 text-mulyo-gold opacity-20 text-8xl md:text-9xl font-serif italic z-[-1]">&</span>
           Romeo <br class="md:hidden"/>
           <span class="text-mulyo-gold hidden md:inline">&amp;</span> Juliet
         </h1>
       </div>

       <div ref="el => { if(el) textRefs[2] = el }" class="opacity-0 invisible">
         <div class="w-[1px] h-16 bg-mulyo-gold mt-6 mb-6 mx-auto"></div>
       </div>

       <div ref="el => { if(el) textRefs[3] = el }" class="opacity-0 invisible">
         <p class="text-mulyo-charcoal/70 font-sans text-xs md:text-sm tracking-[0.2em] font-light max-w-lg leading-relaxed uppercase mb-4">
           We invite you to celebrate<br/>our special day
         </p>
       </div>

       <div ref="el => { if(el) textRefs[4] = el }" class="opacity-0 invisible">
         <p class="text-mulyo-charcoal font-sans text-sm md:text-base tracking-[0.1em] font-medium">
